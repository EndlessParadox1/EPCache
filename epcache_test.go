...
func TestGet(t *testing.T) {
	...
	for k, v := range db {
		if view, err := gen.Get(k); err != nil || view.String() != v {
			t.Fatalf("failed to get value of %s", k)
		}
		if _, err := gen.Get(k); err != nil || loadCounts[k] > 1 {
			t.Fatalf("cache %s miss", k)
		}
	}
	if view, err := gen.Get("unknown"); err == nil {
...
func TestGetGroup(t *testing.T) {
	...
	if group := GetGroup("foo"); group != nil {
